...
export const EpicsList = ({ epics, onCreateEpic }: EpicsListProps) => {
	if (epics.length === 0) {
					...
					>
						<div className="space-y-4">
							...
							<div className="space-y-2">
								<h3 className="text-lg font-bold text-slate-dark leading-snug truncate" title={epic.title}>
									{epic.title}
								</h3>
								{epic.description && (
									<p className="text-slate-medium text-sm line-clamp-3" title={epic.description}>
										{epic.description}
									</p>
								)}
							</div>
						...
						<div className="grid grid-cols-2 gap-4 pt-5 mt-5 border-t border-gray-100/80 text-xs">
							<div>
								<span className="text-[#737685] font-bold uppercase tracking-wider block mb-1.5">Assignee</span>
								{epic.assignee ? (
									<div className="flex items-center gap-2">
										...
										<span className="text-slate-dark font-semibold truncate" title={epic.assignee.name}>
											{epic.assignee.name}
										</span>
									</div>
								) : (
									<span className="text-slate-medium italic leading-8">Unassigned</span>
								)}
							</div>

							<div>
								<span className="text-[#737685] font-bold uppercase tracking-wider block mb-1.5">Created By</span>
								<div className="flex items-center h-8">
									<span className="text-slate-dark font-semibold truncate block" title={epic.created_by.name}>
